...
public class DijkstraPathfinder {
    ...
    public List<Point> findPath(Maze maze) {
        return findPath(maze, null);
    }

    public List<Point> findPath(Maze maze, Point pointToSkip) {
        ...
        while (!queue.isEmpty()) {
            ...
            for (PointDistance nd : maze.reachableNeighbours(u.point())) {
                Point v = nd.point();
                int newDistance;
                if (u.point().equals(pointToSkip)) {
                    newDistance = Integer.MAX_VALUE;
                } else {
                    newDistance = distances.getOrDefault(u.point(), Integer.MAX_VALUE) + nd.distance();
                }
